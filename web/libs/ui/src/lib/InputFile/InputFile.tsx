...
export const InputFile = forwardRef(({ name, className, text, onChange, ...props }: InputFileProps, ref: any) => {
  ...
  const wrapperKeyDownHandler = useCallback((e: any) => {
  ...
  return (
    <label className={clsx(styles.inputWrapper, className)}  tabIndex={0} onKeyDown={wrapperKeyDownHandler}>
      ...
      <input ref={ref} type="file" className={clsx("file-input", styles.input)} name={name} {...props} onChange={onChange} tabIndex={-1}/>
    </label>
  )
});